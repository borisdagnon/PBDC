...
class controleur
{
    ...
    {
        return '    <form id="contact" method="post" action="traitement_formulaire.php">
    ...
    public function retourne_confirmation_mail()
    {
    form="";
    if($_SESSION['type']=='famille')
    {
    $result = $this->vpdo->liste_famille_admin ($_SESSION['id_eleve']);
    $form.='<p>Votre message a bien été envoyé: Nous restons à votre disposition, pour toutes questions</p>';
    }
    <article>
    </article>
    }
...
<br>Nous organisons une activité encadrée tous les soirs</p> 
        ...
        return '
			<article >
				...
				<form id="login" method="post" class="login">
					...
					<input type="radio" name="rblogin" id="rbp"  value="rbp" required/>Personnel
    ...
    public function retourne_commentaire($type, $idEnfant = "")
    {
        $form   = "";
        $prenom = "";
        $nom    = "";
        
        if ($type == 'VoirCommentaire') {
            $titreform   = 'Visualiser les commentaires';
            $libelbutton = '';
        }
        ...
        $form = '
			<article >
				<h3>' . $titreform . '</h3>
				<form id="form_voir_comment" method="post" >';
        
        
        $form .= '
					<p>Nom : <b>' . $nom . '</b></p>
					<p>Prenom : <b>' . $prenom . '</b></p><br/><br/>
					
					';
        
        $result = $this->vpdo->affiche_commentaire($_SESSION['id_eleve']);
        if ($result != false) {
            while ($row = $result->fetch(PDO::FETCH_OBJ))
            // parcourir chaque ligne sélectionnée
                {
                
                $form .= '<div style="width:100%; height:auto; text-align:left;">
					<p style="margin-left:5%;"><b>' . $row->commentaire_date . '</b></p>
					<p style="width:86%; padding:2%; margin-left:5%; background-color:#fff; text-align:justify;">' . $row->commentaire_text . '</p>
					</div><br/>
					';
    ...
    public function retourne_supp_commentaire($type, $idEnfant = "")
    {
        $form   = "";
        $prenom = "";
        $nom    = "";
        ...
        if ($type == 'SuppCommentaire') {
            $titreform   = 'Supression Commentaires enfant';
            $libelbutton = 'Supprimer Commentaires';
        ...
        $form = '
			<article >
				<h3>' . $titreform . '</h3>
				<form id="form_supp_comment" method="post" >
<p>Nom : <b>' . $nom . '</b></p>
					<p>Prenom : <b>' . $prenom . '</b></p><br/>

	Identifiant : <input type="text" name="idEnfant" id="idEnfant" value="' . $_SESSION['id_eleve'] . '" readonly/><br/><br/>
					<select id="dateCommentaire">
				';
        ...
        if ($result != false) {
            ...
            // parcourir chaque ligne sélectionnée
                ...
                $form .= '
					...
					<option value="' . $row->commentaire_date . '">' . $row->commentaire_date . '</option>
	...
	$("#modal").hide();
	//Initialize the tooltips
	$("#form_supp_comment :input").tooltipster({
				         trigger: "custom",
				         onlyOne: false,
				         position: "bottom",
				         multiple:true,
	...
	$("#form_supp_comment").submit(function( e ){
        e.preventDefault();
		$("#modal").hide();
	
		var $url="ajax/valide_supp_commentaire.php";
		if($("#form_supp_comment").valid())
		{
			var formData = {
			"idEnfant"			: $("#idEnfant").val(),
			"dateCommentaire"    : $("#dateCommentaire").val()
			};
    ...
    public function retourne_formulaire_commentaire($type, $idEnfant = "")
    {
        $form   = "";
        $prenom = "";
        $nom    = "";
        
        if ($type == 'AjoutCommentaire') {
            $titreform   = 'Formulaire ajout commentaire';
            $libelbutton = 'Poster';
        }
        ...
        $form .= '
					...
					<p><h3>Mon commentaire du jour : </h3><textarea id="commentaire" name="commentaire" style="width:100%; height:20vh;"></textarea></p>
					<input id="submit" type="submit" name="send" class="button" value="' . $libelbutton . '"/>
					</form>
					';
        ...
        $form = $form . '<script>function hd(){ $(\'#modal\').hide();}</script>
				<div  id="modal" >
										<h1>Informations !</h1>
										<div id="dialog1" ></div>
										<a class="no" onclick="hd();">OK</a>
	...
	$("#formenfant").validate({
		...
		messages:
		{
        	"commentaire":
          	{
            	required: "Vous devez saisir un commentaire valide"
          	}
    ...
    public function retourne_formulaire_enfant($type, $idEnfant = "")
    {
        ...
        if ($type == 'Modif') {
            $titreform   = 'Formulaire Modifier enfant';
            $libelbutton = 'Modifier';
            
        }
        if ($type == 'Supp') {
            $titreform   = 'Formulaire Supprimer enfant';
        ...
        $row = $this->vpdo->liste_enfant_famille_admin($_SESSION['id']);
        ...
        /*
        Cette session permet de vérifier s'il s'agit d'une insertion faite par la famille ou par l'administrateur*/
        if (isset($_SESSION['id']) && isset($_SESSION['type'])) {
            echo "string".'<br/>';
            
            
            if ($_SESSION['type'] == 'admin' && $type == 'Ajout') {
                 
                 $_SESSION['$type']=$type;
                  $row2 = $this->vpdo->liste_famille();
                var_dump($row2);
                if ($row2 != null) {
                    while ($result = $row2->fetch(PDO::FETCH_OBJ))
                    // parcourir chaque ligne sélectionnée
                        {
                        $_SESSION['aka'] = $result->identifiant;
                        echo $_SESSION['aka'];
        ...
        $form = '
			<article >
				<h3>' . $titreform . '</h3>
				<form id="formenfantAjout" method="post" >';
        
        
        
        $form = $form . ' 
					Prénom : <input type="text" name="prenom" id="prenom" placeholder="Nom de l\'enfant" value="' . $prenom . '" required/><br/><br/>
        ...
        $form .= '<script>function hd(){ $(\'#modal\').hide();}</script>
				<div  id="modal" >
										<h1>Informations !</h1>
										<div id="dialog1" ></div>
										<a class="no" onclick="hd();">OK</a>
	...
	$("#modal").hide();
	//Initialize the tooltips
	$("#formenfantAjout :input").tooltipster({
				         trigger: "custom",
				         onlyOne: false,
				         position: "bottom",
				         multiple:true,
	...
	$("#formenfantAjout").submit(function( e ){
        e.preventDefault();
		$("#modal").hide();
	...
	$("#formenfantAjout").validate({
		...
		messages:
		{
        	"prenom":
          	{
            	required: "Vous devez saisir un prénom valide"
          	},
    ...
    public function retourne_formulaire_famille($type, $idfamille = "")
    {
        ...
        if ($type == 'Supp') {
            $titreform   = 'Formulaire Suppression famille';
            $libelbutton = 'Supprimer';
        }
        if ($type == 'Modif') {
    ...
    public function retourne_formulaire_famille_famille($type, $idfamille = "")
    {
        $form        = '';
        $identifiant = '';
        $nom1        = '';
        ...
        if ($type == 'Modif') {
            $row = $this->vpdo->trouve_famille_famille($_SESSION['id']);
            if ($row != null) {
                $identifiant = $row->identifiant;
                $nom1        = $row->nom1;
                $prenom1     = $row->prenom1;
    ...
    public function affiche_liste_enfant_famille_admin($type)
    {
        if ($type == 'liste') {
            $titreform = 'Liste enfant';
    	...
    	<table>
    		...
    		<tbody >';
        if ($type == 'liste') {
            $result = $this->vpdo->liste_enfant_famille_admin($_SESSION['id_eleve']);
        ...
        if ($result != false) {
            ...
            // parcourir chaque ligne sélectionnée
    			...
    			<td Align=center><input onClick="this.form.submit();" type="checkbox" name="nom_checkbox[]" value="' . $row->id . '" /></td>
    			</tr>';
            }
        }
    ...
    public function affiche_liste_enfant_admin($type)
    {
    ...
    public function affiche_liste_enfant($type)
    {
        ...
        if ($type == 'SuppCommentaire') {
            $titreform = 'Supprimer Commentaire Enfant';
        ...
        if ($type == 'AjoutCommentaire' || $type == 'VoirCommentaire' || $type == 'Modif') {
            
            if ($_SESSION['type'] == 'admin') {
                $result = $this->vpdo->liste_enfant();
                echo "C'est l'administrateur qui veut la liste des enfants";
                
            } else {
                if ($_SESSION['type'] == 'famille') {
                    
                    $result = $this->vpdo->liste_enfant_famille_famille($_SESSION['id']);
                    echo "C'est la famille qui veut modifier ces enfants";
        ...
        /*Ici on supprime des enfants ou on supprime des commentaires*/
        else {
            if ($_SESSION['type'] == 'famille') {
                $result = $this->vpdo->liste_enfant_famille_famille($_SESSION['id']);
                echo "C'est la famille qui veut supprimmer ces enfants";
                ;
            } else
                $result = $this->vpdo->liste_enfant();
    ...
    public function affiche_liste_commentaires($type)
    {
        
        if ($type == 'liste') {
            $titreform = 'Liste des Commentaires';
